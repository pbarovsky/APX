...
# Функция для добавления алиасов в .bashrc и .zshrc (если они существуют)
add_aliases_to_shell_config() {
  ...
  for file in "${files[@]}"; do
    if [ -f "$file" ]; then
      # Проверим, не добавлен ли уже алиас
      if ! grep -q "apx-help" "$file"; then
        echo -e "$ALIAS_CONTENT" >> "$file"
        echo "Aliases have been successfully added to $file"
      else
        echo "Aliases have already been added to $file"
      fi
    fi
  done
}

# Выполняем добавление алиасов
add_aliases_to_shell_config
